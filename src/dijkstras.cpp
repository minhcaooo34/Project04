...
int main() {
    ...
    // Output the cost (excluding destination tile cost)
    int totalCost = 0;
    for (size_t i = 1; i < path.size(); ++i) {  // Use size_t to match path.size() type
        totalCost += tileCosts[mapLayout[path[i].first][path[i].second]]; // Cost of leaving the tile
    }
    cout << totalCost << endl;
